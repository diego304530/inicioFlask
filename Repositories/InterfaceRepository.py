...
class InterfaceRepository(Generic[T]):
    def __init__(self):
        ca = certifi.where()
        dataConfig = Config.getConfigJson()
    ...
    def save(self, item: T):
        laColeccion = self.database[self.collection]
        elId = ""
    ...
    def getValuesDBRef(self, x):
        ...
        for k in keys:
            if isinstance(x[k], DBRef):
                laColeccion = self.database[x[k].collection]
    ...
    def getValuesDBRefFromList(self, theList):
        newList = []
        laColeccion = self.database[theList[0]._id.collection]
        for item in theList:
